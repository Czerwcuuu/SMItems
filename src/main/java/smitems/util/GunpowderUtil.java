...
public class GunpowderUtil {
    ...
    public static boolean isGunpowder(ItemStack stack){
        if(stack==null || stack.getType() != Material.GUNPOWDER || !stack.hasItemMeta() || !stack.getItemMeta().hasDisplayName()) return false;

        else if(stack.getItemMeta().getDisplayName().equals("Stylowy Gunpowder")) return true;
    ...
    public static boolean registerGunpowderRecipe(){
        ...
        if(success) SMItems.getInstance().getLogger().fine("Dodany przepis na "+ GUNPOWDER_KEY.getNamespace()+":"+GUNPOWDER_KEY.getKey());
        else SMItems.getInstance().getLogger().fine("NIEPOMYSLNE dodanie przepisu na: "+ GUNPOWDER_KEY.getNamespace()+":"+GUNPOWDER_KEY.getKey());
    ...
    public static boolean unregisterGunpowderRecipe(){
        ...
        if(success) SMItems.getInstance().getLogger().fine("Usunieto przepis na:"+ GUNPOWDER_KEY.getNamespace()+":"+GUNPOWDER_KEY.getKey());

        else SMItems.getInstance().getLogger().fine("Nie udalo sie usunac przepisu na: "+ GUNPOWDER_KEY.getNamespace()+":"+GUNPOWDER_KEY.getKey());
